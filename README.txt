...
The main code is `RunCode.m’ which does the time-stepping, applies the boundary conditions, and calls the functions that 
are required to do portions of the computation. The current configuration solves the nondimensional equations described in
Appendix B for 10 units of time with a surface flux of -0.5 and an accumulation rate of 0.09. The various parameters are
described and comments on lines 19-26. 
...
AccumulationRate : the nondimensional meters of ice accumulated per year (or (1-phi_0) 		times the meters of snow
accumulated)
